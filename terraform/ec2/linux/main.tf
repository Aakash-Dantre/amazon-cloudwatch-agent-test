...
locals {
  ...
  // (the same artifact customers get). Integration test downloads a binary tied to the git
  // hash from the integration bucket.
  ...
  // (--no-sign-request), matching how the canary test itself downloads it. Integration
  // tests read from the private integration bucket and must use signed requests.
